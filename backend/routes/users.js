								...
								function (error, ratingList) {
									if (error) {
										...
										ratingList.forEach((element) =>
											newArray.push(element.doublefeature)
										);
										DoubleFeature.find(
											{
												_id: { $in: newArray },
						...
						function (error, doublefeature) {
							...
							if (error) {
								...
								if (doublefeature.movie_one_id === null) {
									axios
										.get(
											movieURL + doublefeature.movie_two_id + "?api_key=" + key
										)
										.then((response) => {
											const movieOne = [];
									...
									axios
										.get(
											movieURL + doublefeature.movie_one_id + "?api_key=" + key
										)
										.then((response) => {
									...
									const movie_one = axios.get(
										movieURL + doublefeature.movie_one_id + "?api_key=" + key
									);
									const movie_two = axios.get(
										movieURL + doublefeature.movie_two_id + "?api_key=" + key
									);

									axios
										.all([movie_one, movie_two])
										.then(
		...
		DoubleFeature.findOne(
			...
			function (error, doublefeature) {
				if (error) {
					...
					if (doublefeature.movie_one_id === null) {
						axios
							.get(movieURL + doublefeature.movie_two_id + "?api_key=" + key)
							.then((response) => {
								const movieOne = [];
								const movieTwo = response.data;
								let doublefeatureInfo = doublefeature;
						...
						axios
							.get(movieURL + doublefeature.movie_one_id + "?api_key=" + key)
							.then((response) => {
								const movieOne = response.data;
								const movieTwo = [];
								let doublefeatureInfo = doublefeature;
...
router.post("/:username/:identifier/:title", (req, res) => {
	...
	if (req.body.formInstance === "commentform") {
		...
		DoubleFeature.deleteOne({ _id: ObjectID(doublefeatureId) }).then(
			(result) => {
				console.log(result);
			}
		);
		Rating.deleteMany({ doublefeature: ObjectID(doublefeatureId) }).then(
			(result) => {
				console.log(result);
			}
		);
		Comment.deleteMany({ doublefeature: ObjectID(doublefeatureId) }).then(
			(result) => {
				console.log(result);
			}
		);
							...
							DoubleFeature.findOneAndUpdate(
								...
								function (error, doublefeature) {
									if (error) {
										...
										const ratingCount =
											parseInt(doublefeature.rating_count) + 1;
										const ratingValue = parseInt(
											doublefeature.rating_value +
												parseInt(req.body.hiddenRating)
										);
										const ratingAverage = ratingValue / ratingCount;
										const ratingWeighted =
											(ratingCount * ratingAverage + 100 * 6.9) /
											(ratingCount + 100);
										DoubleFeature.findOneAndUpdate(
											{ _id: ObjectID(doublefeatureId) },
											{
